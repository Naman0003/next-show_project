...
import 'package:user_app/services/booking_service.dart';
...
class _EventDetailScreenState extends State<EventDetailScreen> {
  ...
  Future<void> _fetchTMDBData() async {
    try {
      ...
      final dio = Dio();
      final searchResponse = await dio.get(searchUrl.toString());
      if (searchResponse.statusCode == 200) {
        ...
        if (results != null && results.isNotEmpty) {
          ...
          final detailResponse = await dio.get(detailUrl.toString());

          if (detailResponse.statusCode == 200) {
            final movieData = detailResponse.data;
            ...
            final overview = movieData['overview'] ??
                "Aggregated live across independent Berlin cinemas.";
              ...
              if (rawCrew != null) {
                for (var member in rawCrew) {
                  if (member['job'] == 'Director') {
                    directorName =
                        member['name']?.toString() ?? "Independent Director";
                    break;
                  }
                }
              }
    ...
    if (mounted) {
      setState(() {
        ...
        liveOverview =
            "Join us for an incredible live experience featuring top performers and vibrant atmosphere.";
                  ...
                  Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(widget.event.genre,
                            style: const TextStyle(
                                color: Colors.black54, fontSize: 14)),
                        const Divider(height: 32, color: Color(0xFFE2E8F0)),
                        if (castList.isNotEmpty) ...[
                          const Text("Cast & Director",
                              style: TextStyle(
                                  fontSize: 18,
                                  fontWeight: FontWeight.bold,
                                  color: Colors.black87)),
                          const SizedBox(height: 8),
                          Text("Director: $liveDirector",
                              style: const TextStyle(
              ...
              child: ElevatedButton(
                ...
                onPressed: () {
                  Navigator.push(
                    ...
                    MaterialPageRoute(
                        builder: (context) =>
                            CinemaAggregatorScreen(event: widget.event)),
                  );
                },
...
class CinemaAggregatorScreen extends StatelessWidget {
  ...
  const CinemaAggregatorScreen({Key? key, required this.event})
      : super(key: key);
                                ...
                                children: [
                                  ...
                                  Text(st.format,
                                      style: const TextStyle(
                                          ...
                                          fontSize: 10)),
                                  Text("€${st.price.toStringAsFixed(2)}",
                                      style: const TextStyle(
